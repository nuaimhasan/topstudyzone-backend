const Content = require("../../models/admission/content.model");
...
exports.get = async (req, res) => {
  ...
  try {
    ...
    const result = await Content.find(query).populate("subject chapter");
...
exports.getSingle = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await Content.findById(id).populate("subject chapter");
    res.status(200).json({
      success: true,
